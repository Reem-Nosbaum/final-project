...
def login():
	return make_response(jsonify({'page': 'login'}), 200)
...
# @app.route('/flights/<int:id_>')  # get flight by parameters
# def flight_by_parameters(origin_country_id_, destination_country_id_, date_):
# 	repo = repool.get_connection()
# 	anon_facade = AnonymousFacade(repo)
# 	flight_id = anon_facade.get_flights_by_parameters(origin_country_id=origin_country_id_, destination_country_id=destination_country_id_, date=date_)
# 	repool.return_connection(repo)
# 	json_flight_id = flight_id.data_for_web()
# 	return jsonify(json_flight_id)
